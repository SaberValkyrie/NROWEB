<template>
  ...
  <div class="concac" v-if="hpMe > 0 && hpBoss > 0 && playerWin != selectedBey && !End && !start" @click="speed()" :disabled="spinning">
 <div class="text">
  <a class="text-success blinking-text">    Phóng: {{ milis / 1000}}s</a>
  <br>
    <a v-if="round == 1" class="text-danger blinking-text">Nhấn Để Thay Đổi</a>
    <a v-if="round > 1" style="font-weight:bold" class="text-success blinking-text">Tỉ Số: {{ pointMe }} | {{ pointBoss }}</a>

 </div>

  </div>

  <div class="concac" v-else>
  <div class="text">
